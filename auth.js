    ...
    callbacks: {
        async jwt({ token, account, user }) {
            ...
            if (account) {
                ...
                token.refreshToken = account.refresh_token || user?.refreshToken; // Store refresh token for later use
                token.accessTokenExpires = (account.expires_in ? Date.now() + account.expires_in * 1000 : user?.accessTokenExpires) || (Date.now() + 3600 * 1000); // Calculate expiry time
            ...
            // Avoid refreshing if no refresh token exists (e.g. Mock User or Credentials)
