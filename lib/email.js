...
export async function sendNotificationEmail({
  ...
  try {
    return await sendEmailWithRetry({
      ...
      data: {
        ...data,
        // The real domain is engelfinedesign.com — these three fallbacks read
        // "engel**s**finedesign.com", a domain EFD does not own. None of these env vars is set, so
        // the fallback is what actually renders: every email sent carried a dead support address and
        // dead admin/shop links. Absolute URLs are mandatory here — a relative path is inert in an
        // email client.
        supportEmail: process.env.SUPPORT_EMAIL || 'critter@engelfinedesign.com',
        dashboardUrl: process.env.NEXT_PUBLIC_ADMIN_URL || 'https://admin.engelfinedesign.com',
        shopUrl: process.env.NEXT_PUBLIC_SHOP_URL || 'https://shop.engelfinedesign.com'
      }
    });
  } catch (error) {
