...
export const POST = async (req) => {
  ...
  if (body.includeStuller !== false && measured.lengthMm) {
    // PRIMARY: melee/calibrated by mm (covers the bulk + colored stones). Melee must be EXACT
    // (a 2mm slot needs a 2mm stone); center stones get a little grace on size.
    try {
      const res = await searchStonesByMm({ gemType: measured.gemType, cut: measured.cut, lengthMm: measured.lengthMm, widthMm: measured.widthMm, tolerance: big ? 0.5 : 0.2, maxSizes: big ? 3 : 2, maxResults: limit });
    ...
    // small/colored inventory), when the stone is center-sized.
    if (big && isDiamond) {
      try {
        const res = await findStonesByDimensions({ gemType: measured.gemType, cut: measured.cut, lengthMm: measured.lengthMm, widthMm: measured.widthMm, lab: Boolean(body.lab), maxResults: limit });
        stuller.push(...(res.matches || []).map((m) => ({ ...m, kind: 'serialized' })));
