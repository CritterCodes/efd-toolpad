...
import { setInvoiceCashDiscount } from '../../service';
...
export const POST = async (req, { params }) => {
  try {
    ...
    const invoice = await setInvoiceCashDiscount(params.invoiceID, body.enabled !== false);
    return NextResponse.json(invoice, { status: 200 });
  } catch (error) {
    console.error('Error updating cash discount:', error.message);
