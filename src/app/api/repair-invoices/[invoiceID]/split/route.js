...
import { splitInvoice } from '../../service';
...
export const POST = async (req, { params }) => {
  try {
    ...
    const result = await splitInvoice(params.invoiceID, body.repairIDs || []);
    return NextResponse.json(result, { status: 200 });
  } catch (error) {
    console.error('Error splitting repair invoice:', error.message);
