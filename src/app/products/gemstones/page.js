...
export default function GemCutterProductsPage() {
  ...
  const handleCreateProduct = async (productData) => {
    try {
      ...
      // Transform data to new hierarchical structure if needed
      let requestData;
      if (productData.gemstone) {
        // Already in new format
        requestData = productData;
      } else {
        // Transform from flat format to hierarchical
        requestData = {
          ...
          gemstone: {
            ...
            subspecies: productData.subspecies || '',
            carat: Number(productData.carat) || 0,
            dimensions: productData.dimensions || { length: '', width: '', height: '' },
            cut: productData.cut || [],
            cutStyle: productData.cutStyle || [],
            treatment: productData.treatment || [],
            color: productData.color || [],
            locale: productData.locale || '',
            naturalSynthetic: productData.naturalSynthetic || 'natural',
            retailPrice: Number(productData.price || productData.retailPrice) || 0,
            customMounting: productData.customMounting || false,
            vendor: productData.vendor || '',
            internalNotes: productData.notes || productData.internalNotes || '',
            certification: productData.certification || {},
            designCoverage: productData.designCoverage || 'full'
          },
          availableDesigns: productData.availableDesigns || [],
  ...
  const handleUpdateProduct = async (productData) => {
    try {
      ...
      if (productData.gemstone) {
        // Already in new format
        requestData = { ...productData, _id: editingProduct._id };
      } else {
        // Transform from flat format to hierarchical
        requestData = {
          ...
          availableDesigns: productData.availableDesigns || editingProduct.availableDesigns || [],
          relatedProducts: productData.relatedProducts || editingProduct.relatedProducts || []
        };
      }
  ...
  const handleDeleteProduct = async (product) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`/api/products/gemstones?id=${product._id}`, {
        method: 'DELETE'
      });
      ...
      if (data.success) {
        ...
        setError(data.error || 'Failed to delete gemstone');
      }
    } catch (error) {
  ...
  const filteredProducts = products.filter(product => {
    ...
    const species = product.gemstone?.species?.toLowerCase() || '';
    const color = Array.isArray(product.gemstone?.color) 
      ? product.gemstone.color.join(' ').toLowerCase() 
      : (product.gemstone?.color?.toLowerCase() || '');
...
function ProductGrid({ products, onEdit, onDelete, isLoading, emptyMessage }) {
  if (isLoading) {
    return (
  ...
  return (
    <Grid container spacing={3}>
      {products.map((product) => (
        <Grid item xs={12} sm={6} md={4} key={product._id || product.productID}>
          <ProductCard 
            product={product} 
    ...
    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <CardContent sx={{ flexGrow: 1 }}>
        ...
        <Typography variant="h5" color="primary" gutterBottom>
          {formatPrice(gemstoneData.retailPrice || gemstoneData.price || 0)}
        </Typography>
        
        <Box sx={{ mb: 2 }}>
          <Typography variant="body2" color="text.secondary">
            {gemstoneData.carat || gemstoneData.weight || 0}ct • {gemstoneData.cut?.[0] || gemstoneData.shape || ''} • {gemstoneData.species || ''}
          </Typography>
          {gemstoneData.color && (
            <Typography variant="body2" color="text.secondary">
              {Array.isArray(gemstoneData.color) ? gemstoneData.color.join(', ') : gemstoneData.color}
            </Typography>
          )}
          {(gemstoneData.locale || gemstoneData.origin) && (
            <Typography variant="body2" color="text.secondary">
              Origin: {gemstoneData.locale || gemstoneData.origin}
            </Typography>
          )}
        </Box>

        {(product.mountingRequired || gemstoneData.customMounting) && (
          <Chip 
            label={`Mounting: ${product.mountingType || (gemstoneData.customMounting ? 'Custom' : 'Required')}`}
            size="small"
            variant="outlined"
            sx={{ mb: 1 }}
          />
