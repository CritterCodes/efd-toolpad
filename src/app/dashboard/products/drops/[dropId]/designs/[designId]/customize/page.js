...
/**
 * Customizer authoring screen (M3-T2 / decision 0005 §6). Embeds refrakt's
 ...
 *
 * This screen was ORPHANED when Designs moved under Drops: everything downstream (the pricing
 * endpoint, the shop's customize page and PDP gate) kept reading `design.viewer.meshMap`, which
 * nothing wrote any more — so no design could be made customizable at all. Restored here on the
 * current design route, alongside the REFRAKT variant studio.
 */
...
export default function DesignCustomizePage() {
  const { dropId, designId: designID } = useParams();
  ...
  const back = useCallback(
    () => router.push(`/dashboard/products/drops/${design?.dropId || dropId}/designs/${designID}`),
    [router, design?.dropId, dropId, designID],
  );
  ...
  const save = async () => {
    ...
    try {
      ...
      showSnack(
        left.length
          ? `Saved — ${left.length} option(s) still need a cost binding: ${left.join(', ')}`
          : 'Customizer options saved — the listing now offers the customizer.',
  ...
  if (error || !glbUrl) {
    return (
      <Box sx={{ p: 4 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={back} sx={{ color: REPAIRS_UI.textSecondary, mb: 2 }}>Back to design</Button>
  ...
  return (
    <Box sx={{ pb: 6 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 2 }}>
        <Box>
          <Button startIcon={<ArrowBackIcon />} onClick={back} sx={{ color: REPAIRS_UI.textSecondary, mb: 0.5 }}>Back to design</Button>
                      ...
                      <Stack key={val} direction="row" spacing={2} alignItems="center">
                        ...
                        {s.type === 'metal' ? (
                          ...
                          <Autocomplete size="small" sx={{ minWidth: 280 }} options={gemstones}
                            getOptionLabel={(g) => `${g.title || g.productId}${g.gemstone?.carat ? ` · ${g.gemstone.carat}ct` : ''}`}
