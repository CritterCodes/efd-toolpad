 ...
 * spec"). Member-list canvas + persistent header action bar (status chip + readiness summary) +
 * Go-live-now confirm modal + basic scheduling. Readiness GATES RELEASE, not membership (spec §7).
 *
 * Item 4 (#225): this is the canonical `/dashboard/production/drops/[collectionID]` route;
 * `/dashboard/production/collections/[collectionID]` redirects here. API + params (`collectionID`)
 * unchanged — a drop is a collection with a release date.
 */
...
export default function DropDetailPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!cRes.ok) throw new Error(cData.error || 'Failed to load drop');
  ...
  const back = useCallback(() => router.push('/dashboard/production/drops'), [router]);
  ...
  if (error || !collection) {
    return (
      <Box sx={{ p: 4 }}>
        <Button startIcon={<ArrowBackIcon />} onClick={back} sx={{ color: REPAIRS_UI.textSecondary, mb: 2 }}>Back to drops</Button>
        <Typography color="error">{error || 'Drop not found.'}</Typography>
      </Box>
    );
  }
