...
export async function runWeeklyPayroll({ now = new Date(), createdBy = PAYROLL_CRON_ACTOR, notify = true } = {}) {
  ...
  const result = { weekEnd, ownerLedger: [], toPay: [], skipped: [], errors: [] };
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      if (owners.has(candidate.userID)) {
  ...
  try {
    if (result.toPay.length > 0) {
      ...
      await notifyAllAdmins({
        ...
        message: `${result.toPay.length} finalized batch${result.toPay.length === 1 ? '' : 'es'} for ${names}. Pay them and mark paid on the payroll page.`,
