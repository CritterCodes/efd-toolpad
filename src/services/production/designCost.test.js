...
describe('estimateMetalCost', () => {
  it('computes 14k yellow gold metal cost from volume — ×1.3 PLUS the $15 print/sprue labor', () => {
    // 1 cm³ wax → 1 g wax → 13.07 g gold (sg). 24k @ $60/g → 14k = 60*14/24 = $35/g.
    // metal-only = 13.07 * 35 * 1.3 (casting) = 594.685; mounting = + $15 = 609.685
    ...
    expect(r.metalOnlyCost).toBeCloseTo(594.69, 1);
    expect(r.printSetupFee).toBe(15);
    expect(r.metalCost).toBeCloseTo(609.69, 1);
  });

  it('adjusts sterling silver by purity', () => {
    // 2 cm³ → 2 g wax → 20.8 g silver. .999 @ $0.80/g → sterling = 0.80 * 0.925 = $0.74/g.
    // metal-only = 20.8 * 0.74 * 1.3 = 20.0096; mounting = + $15 = 35.0096
    ...
    expect(r.metalCost).toBeCloseTo(35.01, 1);
  });
  ...
  it('yields zero cost when the metal price is missing — no lone $15 masquerading as an estimate', () => {
    const r = estimateMetalCost({ volumeCm3: 1, metalKey: 'GOLD_14K_YELLOW', metalPrices: {} });
    expect(r.metalCost).toBe(0);
    expect(r.printSetupFee).toBe(0);
  });
});
...
describe('estimateDesignCost', () => {
  it('aggregates metal + stones + findings + labor', () => {
    ...
    // material = metalCost(609.69 incl. $15 print/sprue) + 200 + 20 = 829.69; estCost = + 60 = 889.69
    expect(r.estMaterialCost).toBeCloseTo(829.69, 1);
    expect(r.estCost).toBeCloseTo(889.69, 1);
  });
