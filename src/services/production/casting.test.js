...
describe('casting workflow', () => {
  ...
  it('repeated receipt is a no-op and does not duplicate receipt effects', async () => { const received = { pieceID: 'p1', casting: { state: 'received' } }; pieces.findById.mockResolvedValue(received); const result = await markCastingReceived({ pieceID: 'p1', amount: 99 }); expect(result.generation.skipped).toBe('already-received'); expect(pieces.upsertMaterialByCategory).not.toHaveBeenCalled(); expect(expenses.create).not.toHaveBeenCalled(); expect(workOrders.create).not.toHaveBeenCalled(); expect(pieces.transitionCasting).not.toHaveBeenCalled(); });
});
