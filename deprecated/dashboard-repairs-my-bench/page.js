/**
 * DEPRECATED (2026-06) — the legacy repair-only My Bench.
 *
 * Superseded by the unified, source-agnostic bench now served at
 * /dashboard/repairs/my-bench (src/app/dashboard/repairs/my-bench/page.js),
 * which covers repairs + production + customs through one work-order surface.
 * Parked here (outside the app router, so it is NOT a live route) for reference
 * until the unified bench is proven in production; safe to delete thereafter.
 * The /api/repairs/my-bench API it used is still live (the artisan-detail view
 * reads it with a ?userID= param).
 */
"use client";
